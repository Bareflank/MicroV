...
if ($Type -eq "free") {
	Run-MSBuild $solutionpath "builder.sln" $configuration["free"] $platform[$Arch]
}
elseif ($Type -eq "checked") {
	Run-MSBuild $solutionpath "builder.sln" $configuration["checked"] $platform[$Arch]
}
elseif ($Type -eq "sdv") {
	$archivepath = "builder"
	...
	Run-MSBuildSDV $solutionpath "builder" $configuration["sdv"] $platform[$Arch]
