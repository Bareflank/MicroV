...
if ($Type -eq "free") {
	Run-MSBuild $solutionpath "visr.sln" $configuration["free"] $platform[$Arch]
}
elseif ($Type -eq "checked") {
	Run-MSBuild $solutionpath "visr.sln" $configuration["checked"] $platform[$Arch]
}
elseif ($Type -eq "sdv") {
	$archivepath = "visr"
	...
	Run-MSBuildSDV $solutionpath "visr" $configuration["sdv"] $platform[$Arch]
