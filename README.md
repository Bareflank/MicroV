...
- **Cross-Platform Support:** In open source, examples of hypervisors that are capable of
  supporting guest virtual machines include Xen, KVM, and
  ...
  MicroV aims to support as many operating systems as possible including Windows, Linux, UEFI
  and others without sacrificing performance and security. This is accomplished 
  by ensuring the hypervisor is a self-contained binary, independent of the operating 
  system running in the root VM. All of the supporting applications are written in 
  raw C++ to maximize cross-platform support, and any code that is platform specific,
  including drivers, is broken into the platform specific logic (which is minimal) 
  and common logic that can be used across all platforms. 
...
- **Disaggregation and Deprivilege:** None of the above mentioned hypervisors
  ...
  deprivileged hypervisor has yet to be fully realized or supported. MicroV's design 
  starts with a microkernel architecture inside the hypervisor, running most 
  ...
  like Xen and KVM run most, if not all of the hypervisor in Ring 0 of the host. Typically we 
  ...
  a hypervisor. Like the operating system in each guest, the hypervisor can use 
  ...
  leverages the Bareflank microkernel which is a small kernel capable of executing 
  hypervisor extensions in Ring 3 of the host. By itself, Bareflank's microkernel is not 
  capable of executing virtual machines, but instead relies on an extension to
  provide the meat and potatoes. MicroV, at its most basic level is a Bareflank extension 
  that runs in Ring 3, that provides guest virtual machine support. 
...
- **Performance:** Another important focus of the project is on performance.
  ...
  Linux and BSD and therefore are not capable of leveraging some of the
  performance benefits of macOS, Andriod and even Windows such as scheduling and 
  power management, which becomes evident when attempting to use these hypervisors 
  on IoT and mobile devices. Other 
  approaches to microkernel hypervisors attempt to provide their own 
  ...
  operating system that manages that device. 

- **Virtual Device Support:** Hypervisors like KVM manage all physical devices in 
  Ring 0 of the host (the most privileged code on the system) using the Linux kernel. 
  This is one way in which Xen is 
  ...
  operating system to manage the physical devices it is given. All virtual 
  device backends run in Ring 0 or Ring 3 of the guest root VM, which is the 
  main virtual machine on the system. 
...
- **Scheduling:** Although closely related to performance, MicroV leverages a hybrid
  ...
  relying on the root VM's operating system to schedule each VM along with the rest of the critical
  ...
  present in modern operating systems. Since MicroV is designed with cross-platform 
  in mind, this also means that support for custom schedulers is possible, including 
  RTOS schedulers. 
...
- **AUTOSAR Compliant** Although Xen, KVM and VirtualBox provide various
  ...
  possible to support standards such as ISO 26262. In addition, MicroV was re-engineered from the ground up using 
...
- **Licensing:** Most of the hypervisors available today in open source leverage
  ...
  We also love pull requests, RFCs, bug reports and feature requests. 
