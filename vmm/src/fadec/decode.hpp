...
fd_decode(const uint8_t *buffer, size_t len_sz, int mode_int, uintptr_t address, FdInstr *instr)
{
    ...
    switch (mode_int) {
        ...
        case 32:
            table_idx = FD_TABLE_OFFSET_32;
            mode = DECODE_32;
            break;
#endif
#if defined(FD_TABLE_OFFSET_64)
        case 64:
            table_idx = FD_TABLE_OFFSET_64;
            mode = DECODE_64;
            break;
#endif
        default:
    ...
    if (mode == DECODE_32) {
        while (LIKELY(off < len)) {
            ...
            switch (UNLIKELY(prefix)) {
                ...
                case 0x26:
                    instr->segment = FD_REG_ES;
                    break;
                case 0x2e:
                    instr->segment = FD_REG_CS;
                    break;
                case 0x36:
                    instr->segment = FD_REG_SS;
                    break;
                case 0x3e:
                    instr->segment = FD_REG_DS;
                    break;
                case 0x64:
                    instr->segment = FD_REG_FS;
                    break;
                case 0x65:
                    instr->segment = FD_REG_GS;
                    break;
                case 0x66:
                    prefix_66 = true;
                    break;
...
#undef FD_DECODE_TABLE_DESCS
    ...
    instr->flags = prefix_rep == 2 ? FD_FLAG_REP : prefix_rep == 3 ? FD_FLAG_REPNZ : 0;
    if (mode == DECODE_64)
...
skip_modrm:
    ...
    if (UNLIKELY(imm_control == 1)) {
        ...
        if (imm_byte)
            imm_size = 1;
                     ...
                     instr->type == FDI_RET || instr->type == FDI_RETF ||
    ...
    uint8_t operand_sizes[4] = {
        static_cast<uint8_t>((1 << DESC_SIZE_FIX1(desc)) >> 1),
        static_cast<uint8_t>(1 << DESC_SIZE_FIX2(desc)),
        static_cast<uint8_t>(op_size),
        static_cast<uint8_t>(vec_size)};
